@if(isset($is_pdf))
    {{-- ============================================================== --}}
    {{-- TAMPILAN KHUSUS CETAK PDF (Jalan saat tombol cetak diklik)     --}}
    {{-- ============================================================== --}}
    <!DOCTYPE html>
    <html lang="id">
    <head>
        <meta charset="UTF-8" />
        <title>Rekap Nilai Tugas - {{ $kelas->kode_kelas }}</title>
        <style>
            @page { margin: 30px 40px; }
            body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #1e293b; font-size: 11px; }
            ...
            .badge-kelas {background-color: #dbeafe; color: #1d4ed8; padding: 3px 8px; border-radius: 4px; font-size: 10px; font-weight: bold;}
            .data-table { width: 100%; border-collapse: collapse; }
            .data-table th, .data-table td { border: 1px solid #cbd5e1; padding: 8px; text-align: center; vertical-align: middle; }
            .data-table th { background-color: #f8fafc; color: #475569; text-transform: uppercase; font-size: 9px; font-weight: bold; }
            .text-left { text-align: left !important; }
            .mhs-nama { font-weight: bold; color: #0f172a; font-size: 11px; }
            .mhs-nim { color: #64748b; font-size: 9px; margin-top: 2px; }
            .badge {display: inline-block; padding: 4px 0; width: 35px; border-radius: 4px; font-weight: bold; font-size: 10px; text-align: center;}
            .badge-emerald { background-color: #ecfdf5; color: #047857; border: 1px solid #a7f3d0; }
            .badge-slate { background-color: #f1f5f9; color: #94a3b8; border: 1px solid #e2e8f0; }
    ...
    <body>
            ...
            <table style="width: 100%;">
                <tr>
                    <td style="text-align: left;">
                        ...
                        <p>Portal Dosen - Sistem Informasi Akademik</p>
                    </td>
                    <td style="text-align: right; vertical-align: bottom;">
                        <span class="badge-kelas">KELAS {{ $kelas->kode_kelas }}</span>
                    </td>
                </tr>
            </table>
        </div>
        ...
        <table class="info-table">
            <tr>
                ...
                <td>: <strong>{{ $kelas->mataKuliah->nama ?? '-' }}</strong></td>
            </tr>
            <tr>
            ...
            <tr>
                <td class="info-label">Total Tugas</td>
                <td>: {{ $assignments->count() }} Tugas</td>
            </tr>
        </table>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th class="text-left" style="width: 150px;">Mahasiswa</th>
                    @foreach($assignments as $index => $tugas)
                        <th>
                            <div style="color: #2563eb; margin-bottom: 2px;">TGS {{ $index + 1 }}</div>
                            <div style="font-size: 7px; color: #94a3b8; font-weight: normal;">{{ Str::limit($tugas->judul, 10) }}</div>
                        </th>
                    @endforeach
                    <th style="background-color: #e0f2fe; color: #0369a1; width: 50px;">Rata-rata</th>
                </tr>
            </thead>
            <tbody>
                @forelse($kelas->mahasiswa as $index => $mhs)
                    ...
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td class="text-left">
                            <div class="mhs-nama">{{ $mhs->nama }}</div>
                        ...
                        @foreach($assignments as $tugas)
                            @php
                                ...
                                if($nilai !== null) { $totalNilai += $nilai; $jumlahDinilai++; }
                            @endphp
                            
                            <td>
                                @if($nilai !== null)
                                    <span class="badge badge-emerald">{{ $nilai }}</span>
                                @else
                        ...
                        @php
                            ...
                            if($rataRata >= 80) $badgeColor = 'badge-blue';
                            elseif($rataRata >= 60) $badgeColor = 'badge-amber';
                            elseif($rataRata > 0) $badgeColor = 'badge-red';
                        @endphp
                ...
                @empty
                    <tr>
                        <td colspan="{{ $assignments->count() + 3 }}" style="padding: 20px; color: #94a3b8;">
...
@else
    {{-- ============================================================== --}}
    {{-- TAMPILAN WEB UTAMA DENGAN TAILWIND (Jalan saat buka di browser)--}}
    {{-- ============================================================== --}}
    <!DOCTYPE html>
    ...
    <body class="font-['Plus_Jakarta_Sans'] bg-[#f1f5f9] text-slate-800 min-h-full flex flex-col border-box overflow-x-hidden selection:bg-blue-200">
        ...
        <div class="w-full bg-white/80 backdrop-blur-2xl border-b border-slate-200/60 sticky top-0 z-40 px-4 md:px-8 py-4 shadow-sm transition-all">
            <div class="max-w-7xl mx-auto flex items-center justify-between gap-5">
                ...
                {{-- TOMBOL CETAK PDF --}}
                <a href="{{ route('dosen.assignment.recap.pdf', $kelas->id) }}" class="hidden md:flex px-5 py-2.5 bg-slate-800 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-900 transition-all items-center gap-2 shadow-md">
