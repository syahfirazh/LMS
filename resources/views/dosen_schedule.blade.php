...
<head>
    ...
    <style>
        ...
        /* Pengaturan Khusus PDF / Kertas Print */
        @media print {
            ...
            * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
            ...
            .print-table th, .print-table td { border: 1px solid #334155; padding: 10px 8px; vertical-align: middle; }
            .print-table th { background-color: #f1f5f9 !important; color: #0f172a; text-transform: uppercase; font-size: 11px; text-align: center; }
            .print-table td.day-header { background-color: #f8fafc !important; font-weight: bold; text-align: center; text-transform: uppercase; }
        }
    </style>
</head>
<body class="m-0 font-['Plus_Jakarta_Sans'] bg-slate-50 text-slate-800 antialiased h-screen flex overflow-hidden print:h-auto print:bg-white print:overflow-visible">
    
    <div id="mobileBackdrop" onclick="toggleSidebar()" class="fixed inset-0 bg-slate-900/50 z-40 hidden lg:hidden transition-opacity print:hidden"></div>

    {{-- SIDEBAR UTAMA (Tetap Utuh, Hanya Hilang Saat di-Print) --}}
    <aside id="sidebar" class="fixed lg:static inset-y-0 left-0 z-50 w-80 bg-white border-r border-slate-200 flex flex-col h-full transform -translate-x-full lg:translate-x-0 transition-transform duration-300 shrink-0 shadow-2xl lg:shadow-none print:hidden">
        ...
        <div class="p-6 border-t border-slate-100 shrink-0 print:hidden">
    ...
    <main class="flex-1 flex flex-col h-full relative min-w-0 overflow-y-auto custom-scrollbar print:overflow-visible print:w-full">
        
        {{-- ========================================================== --}}
        {{-- TAMPILAN WEB (KARTU JADWAL)                                --}}
        {{-- ========================================================== --}}
        <div class="print:hidden">
            <header class="bg-white/80 backdrop-blur-xl border-b border-slate-200/60 px-4 md:px-8 py-4 sm:py-6 sticky top-0 z-30 flex items-center justify-between">
                ...
                <div class="hidden md:flex items-center gap-4">
                    <button onclick="window.print()" class="flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-slate-700 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 cursor-pointer">
        ...
        {{-- ========================================================== --}}
        {{-- TAMPILAN PDF (TABEL) HANYA MUNCUL SAAT PRINT                 --}}
        {{-- ========================================================== --}}
        <div class="hidden print:block w-full text-black">
            <div style="text-align: center; border-bottom: 2px solid #1e293b; padding-bottom: 15px; margin-bottom: 20px;">
                <h1 style="margin: 0; font-size: 20px; text-transform: uppercase; letter-spacing: 1px; color: #0f172a;">Jadwal Mengajar Dosen</h1>
                <p style="margin: 5px 0 0 0; color: #475569; font-size: 12px; font-weight: bold;">Semester Akademik Berjalan</p>
            </div>

            <div style="margin-bottom: 20px; font-size: 12px;">
                <table style="width: auto; border: none; margin: 0;">
                    <tr>
                        <td style="padding: 3px 10px 3px 0; border: none; font-weight: bold; width: 100px;">Nama Dosen</td>
                        <td style="padding: 3px 0; border: none;">: <strong>{{ auth('dosen')->user()->nama ?? '-' }}</strong></td>
                    </tr>
                    <tr>
                        <td style="padding: 3px 10px 3px 0; border: none; font-weight: bold;">NIDN</td>
                        <td style="padding: 3px 0; border: none;">: {{ auth('dosen')->user()->nidn ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td style="padding: 3px 10px 3px 0; border: none; font-weight: bold;">Tanggal Cetak</td>
                        <td style="padding: 3px 0; border: none;">: {{ \Carbon\Carbon::now()->format('d/m/Y') }}</td>
                    </tr>
                </table>
            </div>
            ...
            <table class="print-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 40%; text-align: left; padding-left: 10px;">Mata Kuliah</th>
                        <th style="width: 10%;">Kelas</th>
                        <th style="width: 15%;">Ruang</th>
                    </tr>
                </thead>
                <tbody>
                    @php $hasJadwalPrint = false; @endphp
                    
                    @forelse($jadwalPerHari as $hari => $kelasHariIni)
                        @if(count($kelasHariIni) > 0)
                            @php $hasJadwalPrint = true; @endphp
                            @foreach($kelasHariIni as $index => $kelas)
                                <tr>
                                    @if($index == 0)
                                        <td rowspan="{{ count($kelasHariIni) }}" class="day-header">{{ $hari }}</td>
                                    @endif
                                    <td style="text-align: center; font-weight: bold;">{{ $kelas->jam_mulai }} - {{ $kelas->jam_selesai }}</td>
                                    <td style="font-weight: bold; color: #0f172a; padding-left: 10px;">{{ $kelas->mataKuliah->nama ?? '-' }}</td>
                                    <td style="text-align: center;">{{ $kelas->kode_kelas }}</td>
                                    <td style="text-align: center;">{{ $kelas->ruangan }}</td>
                                </tr>
                            @endforeach
                        @endif
                    @empty
                    ...
                    @if(!$hasJadwalPrint)
                        <tr>
                            <td colspan="5" style="text-align: center; font-style: italic; color: #64748b; padding: 20px;">Belum ada jadwal mengajar yang terdaftar.</td>
                        </tr>
                    @endif
                </tbody>
            </table>
