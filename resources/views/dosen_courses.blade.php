...
<body class="m-0 font-['Plus_Jakarta_Sans'] bg-[#f8fafc] min-h-full flex flex-col lg:flex-row overflow-hidden border-box text-slate-800">
    ...
    <main class="flex-1 h-screen overflow-y-auto flex flex-col relative bg-[#f8fafc] lg:ml-80 transition-all duration-300">
        ...
        <header class="bg-white/80 backdrop-blur-xl border-b border-slate-200/60 px-8 py-6 sticky top-0 z-30">
            <div class="max-w-7xl mx-auto flex items-center justify-between h-14">
                <div class="flex items-center gap-4">
                    <button onclick="toggleSidebar()" class="lg:hidden p-2 text-slate-500 hover:bg-slate-100 rounded-lg transition-all focus:outline-none focus:ring-2 focus:ring-slate-200">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                    </button>
                    <div>
                        <h2 class="text-2xl font-black text-slate-900 tracking-tight">Mata Kuliah Diampu</h2>
                        <p class="text-sm font-medium text-slate-500">Semester Ganjil 2025/2026</p>
                    </div>
                </div>
        ...
        <div class="p-6 lg:p-10 w-full max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            @forelse ($kelasDiampu ?? [] as $kelas)
                <div data-aos="fade-up" data-aos-duration="800" data-aos-delay="{{ 100 + ($loop->index * 100) }}"
                     class="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group relative overflow-hidden flex flex-col">
                    ...
                    <div class="p-8 flex-1 flex flex-col">
                        ...
                        <div class="mt-auto pt-6 border-t border-slate-100 flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-2">
                                <button type="button" onclick="openEditModal({{ json_encode($kelas) }})" class="p-2 rounded-lg text-slate-300 hover:bg-blue-50 hover:text-blue-500 transition-all cursor-pointer" title="Edit Kelas">
                                ...
                                <form action="{{ route('dosen.kelas.destroy', $kelas->id) }}" method="POST" onsubmit="return confirm('Hapus kelas ini?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="p-2 rounded-lg text-slate-300 hover:bg-red-50 hover:text-red-500 transition-all cursor-pointer" title="Hapus Kelas">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                    </button>
                                </form>

                                <a href="{{ route('dosen.course.manage', $kelas->id) }}" class="ml-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold hover:bg-blue-600 hover:text-white transition-all cursor-pointer">
                                    Kelola
                                </a>
                            </div>
                        </div>
            ...
            @empty
                <div data-aos="fade-up" data-aos-duration="800" data-aos-delay="200"
                     class="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group relative overflow-hidden flex flex-col">
                    <div class="h-32 w-full bg-gradient-to-r from-blue-100 to-blue-200 relative overflow-hidden">
                        <div class="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-lg shadow-sm">
                            <span class="text-xs font-black text-slate-700 uppercase tracking-widest">3A</span>
                        </div>
                    </div>
                    <div class="p-8 flex-1 flex flex-col">
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                <h3 class="text-xl font-black text-slate-900 mb-1 group-hover:text-blue-600 transition-colors line-clamp-2">Pemrograman Berorientasi Objek</h3>
                                <p class="text-sm text-slate-500 font-medium flex items-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    Senin, 08:00 - 10:30
                                </p>
                        ...
                        <div class="mt-auto pt-6 border-t border-slate-100 flex items-center justify-between">
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">35 Mahasiswa</span>
                            <div class="flex items-center gap-2">
                                <a href="#" class="ml-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold hover:bg-blue-600 hover:text-white transition-all cursor-pointer">Kelola</a>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div id="modalKelas" class="fixed inset-0 z-[100] hidden">
        ...
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl transform scale-100 transition-transform relative overflow-hidden">
                <form action="{{ route('dosen.kelas.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="px-8 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <h3 class="text-lg font-black text-slate-800">Buat Kelas Baru</h3>
                        <button type="button" onclick="toggleModal('modalKelas', false)" class="text-slate-400 hover:text-red-500 transition-colors cursor-pointer">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                        </button>
                    </div>

                    <div class="p-6 space-y-4">
                        <div><label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Nama Mata Kuliah</label><input type="text" name="nama_mata_kuliah" placeholder="Contoh: Algoritma" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 font-bold text-slate-700 text-sm focus:ring-2 focus:ring-blue-500 outline-none" /></div>
                        <div class="grid grid-cols-3 gap-4">
                            <div><label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Kode</label><input type="text" name="kode_kelas" placeholder="3A" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 font-bold text-slate-700 text-sm outline-none" /></div>
                            <div><label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">SKS</label><input type="number" name="sks" min="1" max="6" value="3" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 font-bold text-slate-700 text-sm outline-none" /></div>
                            <div>
                        ...
                        <div class="flex items-center gap-4 bg-slate-50 p-3 rounded-xl border border-dashed border-slate-300">
                            <div class="shrink-0 bg-white p-2 rounded-lg text-slate-400">
                    ...
                    <div class="px-8 py-5 bg-slate-50 flex justify-end gap-3 rounded-b-[2rem]">
   ...
   <div id="modalEditKelas" class="fixed inset-0 z-[100] hidden">
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-xl transform scale-100 transition-transform relative overflow-hidden">
            
            <form id="formEditKelas" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT') 

                <div class="px-8 py-5 border-b border-slate-100 flex justify-between items-center bg-yellow-50/50">
                    <h3 class="text-lg font-black text-slate-800">Edit Kelas</h3>
                    <button type="button" onclick="toggleModal('modalEditKelas', false)" class="text-slate-400 hover:text-red-500 transition-colors cursor-pointer">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div>
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Kode</label>
                            <input type="text" id="edit_kode" name="kode_kelas" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 font-bold text-slate-700 text-sm outline-none" />
                        </div>
                        <div>
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">SKS</label>
                            <input type="number" name="sks" value="3" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 font-bold text-slate-700 text-sm outline-none bg-slate-100 text-slate-400" disabled />
                        </div>
                        <div>
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Hari</label>
