...
<head>
    ...
    <style>
        ...
        /* ========================================================== */
        /* PENGATURAN KHUSUS SAAT CETAK / SAVE TO PDF                 */
        /* ========================================================== */
        @media print {
...
<body class="font-['Plus_Jakarta_Sans'] bg-[#f1f5f9] text-slate-800 min-h-full flex flex-col border-box overflow-x-hidden selection:bg-blue-200 print:bg-white print:overflow-visible">
    ...
    {{-- ============================================================== --}}
    {{-- 2. BAGIAN INI HANYA MUNCUL SAAT DI-PRINT (BERBENTUK TABEL)       --}}
    {{-- ============================================================== --}}
    <div class="hidden print-only w-full max-w-none text-black">
            ...
            <table style="width: 100%; border: none;">
                <tr>
                    <td style="text-align: left; border: none; padding: 0;">
                        <h1 style="font-size: 20px; margin: 0; color: #0f172a; text-transform: uppercase; letter-spacing: 1px;">Rekap Nilai Tugas</h1>
        ...
        {{-- INFO KELAS --}}
        <table style="width: 100%; margin-bottom: 20px; font-size: 12px; border: none;">
            <tr>
                <td style="width: 130px; font-weight: bold; color: #475569; padding: 4px 0; border: none;">Mata Kuliah</td>
                <td style="padding: 4px 0; border: none;">: <strong>{{ $kelas->mataKuliah->nama ?? '-' }}</strong></td>
            </tr>
            <tr>
                <td style="font-weight: bold; color: #475569; padding: 4px 0; border: none;">Dosen Pengampu</td>
                <td style="padding: 4px 0; border: none;">: {{ $kelas->dosen->nama ?? '-' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold; color: #475569; padding: 4px 0; border: none;">Total Tugas</td>
                <td style="padding: 4px 0; border: none;">: {{ $assignments->count() }} Tugas</td>
            </tr>
        </table>
        ...
        <table class="print-table">
            <thead>
                <tr>
                    ...
                    <th class="text-left" style="width: 150px;">NIM & Nama Mahasiswa</th>
                    @foreach($assignments as $index => $tugas)
                        <th>
                            <div style="color: #1e40af; margin-bottom: 2px;">TGS {{ $index + 1 }}</div>
                            <div style="font-size: 8px; color: #64748b; font-weight: normal;">{{ Str::limit($tugas->judul, 15) }}</div>
                        </th>
                    @endforeach
                    <th style="background-color: #e0f2fe !important; color: #0369a1; width: 50px;">Rata-rata</th>
                </tr>
            </thead>
            <tbody>
                @forelse($kelas->mahasiswa as $index => $mhs)
                    ...
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td class="text-left">
                            <div style="font-weight: bold; color: #0f172a; font-size: 11px;">{{ $mhs->nim }}</div>
                            <div style="color: #475569; font-size: 11px; margin-top: 2px;">{{ $mhs->nama }}</div>
                        </td>
                        
                        @foreach($assignments as $tugas)
                            @php
                                $submissionPdf = $tugas->submissions->where('mahasiswa_id', $mhs->id)->first();
                                $nilaiPdf = $submissionPdf ? $submissionPdf->nilai : null;
                                if($nilaiPdf !== null) { $totalNilaiPdf += $nilaiPdf; $jumlahDinilaiPdf++; }
                            @endphp
                            
                            <td>
                                @if($nilaiPdf !== null)
                                    <strong>{{ $nilaiPdf }}</strong>
                                @else
                        ...
                        @php
                            $rataRataPdf = $jumlahDinilaiPdf > 0 ? round($totalNilaiPdf / $assignments->count(), 1) : 0;
                        @endphp
                        
                        <td style="background-color: #f0f9ff; font-weight: bold; font-size: 12px; color: #0369a1;">
                            {{ $rataRataPdf > 0 ? $rataRataPdf : '-' }}
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="{{ $assignments->count() + 3 }}" style="padding: 20px; color: #94a3b8; text-align: center; font-style: italic;">
                            Belum ada data mahasiswa di kelas ini.
                        </td>
                    </tr>
                @endforelse
