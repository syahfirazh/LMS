...
<html lang="id" class="h-full">
    ...
    <body class="m-0 font-['Plus_Jakarta_Sans'] bg-[#f8fafc] min-h-full flex flex-col lg:flex-row overflow-hidden border-box text-slate-800">
        ...
        <main class="flex-1 h-screen overflow-y-auto flex flex-col relative lg:ml-80 transition-all duration-300">
            ...
            <header class="bg-white/80 backdrop-blur-xl border-b border-slate-200/60 px-8 py-6 sticky top-0 z-30">
                <div class="max-w-7xl mx-auto flex items-center justify-between h-14">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-2xl font-black text-slate-900 tracking-tight">Halo, Pak {{ explode(' ', $dosen->nama)[0] }}</h2>
                            <p class="text-sm font-medium text-slate-500">{{ $tanggalLengkap ?? 'Hari ini' }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-4">
                        <div
                        ...
                        >
                            <div class="text-right hidden md:block">
                                ...
                                >
                                    NIDN:
                                    {{ Auth::guard('dosen')->user()->nidn ?? '0000' }}
                                </p>
                            </div>
                            <div
                            ...
                            >
                                <img
                                    src="https://ui-avatars.com/api/?name={{ urlencode(Auth::guard('dosen')->user()->nama ?? 'Dosen') }}&background=0D8ABC&color=fff"
                                    alt="Profile"
            ...
            <div class="p-6 lg:p-10 max-w-7xl mx-auto w-full space-y-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div data-aos="fade-up" data-aos-delay="100" class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-4 hover:shadow-md transition-all">
                        <div class="w-14 h-14 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center font-black text-xl">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Kelas Diampu</p>
                            <h3 class="text-2xl font-black text-slate-900">{{ $totalMatkul ?? 0 }} Kelas</h3>
                        </div>
                    </div>
                    ...
                    <div data-aos="fade-up" data-aos-delay="200" class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-4 hover:shadow-md transition-all">
                        <div class="w-14 h-14 bg-orange-50 text-orange-600 rounded-2xl flex items-center justify-center font-black text-xl">
                        ...
                        <div>
                            ...
                            <h3 class="text-2xl font-black text-slate-900">{{ $totalMahasiswa ?? 0 }}</h3>
                        </div>
                    </div>

                    <div data-aos="fade-up" data-aos-delay="300" class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-4 hover:shadow-md transition-all">
                        <div class="w-14 h-14 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center font-black text-xl">
                        ...
                        <div>
                            ...
                            <h3 class="text-2xl font-black text-slate-900">{{ $totalPenilaian ?? 0 }} Tugas</h3>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div class="lg:col-span-2 flex flex-col gap-6">
                        <div class="flex justify-between items-center px-2" data-aos="fade-left" data-aos-delay="400">
                            <h3 class="text-lg font-black text-slate-900 uppercase tracking-wide">Mata Kuliah Diampu</h3>
                        ...
                        <div class="flex flex-col gap-4">
                            ...
                            <div data-aos="fade-up" data-aos-delay="{{ $delay }}"
                                 class="group bg-white p-5 rounded-[1.5rem] border border-slate-200 shadow-sm hover:border-{{ $color }}-300 hover:shadow-md transition-all cursor-pointer flex flex-col md:flex-row items-center gap-6 justify-between">
                                
                                <div class="flex items-center gap-5 w-full md:w-auto">
                                    <div class="w-14 h-14 bg-{{ $color }}-100 text-{{ $color }}-600 rounded-2xl flex items-center justify-center shrink-0 font-black text-lg group-hover:bg-{{ $color }}-600 group-hover:text-white transition-all">
                                        {{ strtoupper(substr($kelas->mataKuliah->nama ?? 'MK', 0, 2)) }}
                                    </div>
                                    <div>
                                        <h4 class="text-base font-black text-slate-900 group-hover:text-{{ $color }}-700 transition-colors">
                                            {{ $kelas->mataKuliah->nama ?? 'Mata Kuliah' }}
                                        </h4>
                                        <div class="flex gap-2 mt-1">
                                            <span class="px-2 py-0.5 bg-slate-100 rounded-md text-[10px] font-bold uppercase tracking-widest text-slate-500">
                                                Kelas {{ $kelas->kode_kelas ?? '-' }}
                                            </span>
                                            <span class="hidden md:inline-block px-2 py-0.5 bg-{{ $color }}-50 text-{{ $color }}-600 rounded-md text-[10px] font-bold uppercase tracking-widest border border-{{ $color }}-100">
                                                {{ $kelas->sks ?? 3 }} SKS
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <div class="flex items-center justify-between w-full md:w-auto gap-8 border-t md:border-t-0 border-slate-100 pt-3 md:pt-0">
                                    ...
                                    <div class="flex items-center gap-4">
                                        <span class="text-[10px] font-bold text-slate-400 bg-slate-50 px-3 py-1 rounded-lg border border-slate-100">
                                            {{ $kelas->mahasiswa->count() }} Mhs
                                        </span>
                                        <div class="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center group-hover:bg-{{ $color }}-50 transition-all shrink-0">
                            ...
                            <div class="p-8 text-center bg-white rounded-[2rem] border border-dashed border-slate-300">
                                <p class="text-slate-500 font-bold">Belum ada kelas yang diampu.</p>
                            </div>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div data-aos="fade-left" data-aos-delay="600" data-aos-duration="800">
                        <h3 class="text-lg font-black text-slate-900 uppercase tracking-wide mb-6">Mengajar Hari Ini</h3>
                        ...
                        <div class="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
                            ...
                            <div class="flex gap-4 relative">
                                ...
                                <div class="pb-6">
                                    <span class="text-xs font-bold {{ $jadwal['status'] == 'berlangsung' ? 'text-blue-600' : 'text-slate-400' }}">
                                        {{ $jadwal['jam_mulai'] }} - {{ $jadwal['jam_selesai'] }}
                                    </span>
                                    <h4 class="font-bold text-sm mt-1 {{ $jadwal['status'] == 'berlangsung' ? 'text-slate-900' : 'text-slate-500' }}">
                                        {{ $jadwal['mata_kuliah'] }} 
                                        <span class="font-normal text-slate-400">({{ $jadwal['kelas'] }})</span>
                                    </h4>
                                    <p class="text-xs text-slate-500">{{ $jadwal['ruangan'] ?? 'Online' }}</p>
                                    @if($jadwal['status'] == 'berlangsung')
                                    <span class="inline-block mt-2 px-2 py-1 bg-emerald-100 text-emerald-700 text-[9px] font-bold rounded uppercase">Sedang Berlangsung</span>
                                    @endif
                                </div>
                            </div>
                            @empty
                            <div class="text-center py-8">
                                ...
                                <p class="text-sm font-bold text-slate-900">Tidak ada jadwal hari ini.</p>
                                <p class="text-xs text-slate-400 mt-1">Istirahatlah sejenak, Pak!</p>
                            </div>
                            @endforelse
                        </div>

                        <div class="mt-6 bg-orange-50 p-6 rounded-[2rem] border border-orange-100">
                            <div class="flex gap-3">
                                <div class="p-2 bg-white rounded-lg text-orange-500 shadow-sm h-fit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                </div>
                                <div>
                                    <h4 class="font-bold text-orange-800 text-sm">Batas Waktu Penilaian</h4>
                                    <p class="text-xs text-orange-600 mt-1 leading-relaxed">Segera input nilai sebelum periode berakhir.</p>
                                </div>
                            </div>
                        </div>
                    </div>
